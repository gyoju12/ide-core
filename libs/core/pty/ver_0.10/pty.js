...
if (/arm/.test(os.arch())) {
  pty = require('./arm/pty.node');
}
else {
  if(/darwin/.test(os.platform())) {
    pty = require('./darwin/pty.node');
  }
  else if(/linux/.test(os.platform())) {
    if(/ia32/.test(os.arch())) 
      pty = require('./linux/x86/pty.node');
    else pty = require('./linux/x64/pty.node');
  }
}
/**
 * Terminal
...
Terminal.prototype.resize = function(cols, rows) {
  ...
  try {
    ...
    console.log('Error: ', this.fd, cols, rows);
  }
};
