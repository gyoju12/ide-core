(function() {
  ...
  $.widget("ui.dialogExtend", {
    ...
    _initButtons: function() {
      ...
      // titlebar.append('<a class="ui-dialog-titlebar-restore ui-corner-all ui-state-default" href="#"><span class="ui-icon ' + this.options.icons.restore + '">restore</span></a>').find('.ui-dialog-titlebar-restore').attr("role", "button").mouseover(function() {
      titlebar.append('<button class="ui-dialog-titlebar-restore></button>').mouseover(function() {
    ...
    _initModuleButton: function(name, mode) {
      ...
      titlebar = $(this.element[0]).dialog("widget").find(".ui-dialog-titlebar");
      return titlebar.append('<button class="ui-dialog-titlebar-' + name + '"></button>').mouseover(function() {
        return $(this).addClass("ui-state-hover");
      }).mouseout(function() {
        return $(this).removeClass("ui-state-hover");
      }).focus(function() {
...
//       });
    ...
    _restore_maximized: function() {
      ...
      $(this.element[0]).dialog("widget").css("position", original.position.mode).find(".ui-dialog-titlebar").css("white-space", original.titlebar.wrap).end().find(".ui-dialog-titlebar").show().end().find(".ui-dialog-content").dialog("option", {
        "resizable": original.config.resizable,
      ...
      if ($(this.element[0]).dialog("option", "draggable")) {
        ...
        return this._trigger("_restore_maximized");
      }
    }
...
(function() {
  ...
  $.extend(true, $.ui.dialogExtend.prototype, {
    ...
    minimize: function() {
      ...
      // this statement is added to customize...
      $(this.element[0]).dialog("widget").hide();
    ...
    _restore_minimized: function() {
      ...
      $(this.element[0]).dialog("widget").appendTo("#workspace").css({
        "float": "none",
        "margin": 0,
        "position": original.position.mode
      }).find(".ui-dialog-content").dialog("widget").find(".ui-dialog-title").css("width", "100%").end().find(".ui-dialog-content").show().dialog("widget")/*.find(".ui-dialog-buttonpane:hidden").show().end()*/.find(".ui-dialog-titlebar").css("white-space", original.titlebar.wrap).end().find(".ui-dialog-content").dialog("option", {
        "resizable": original.config.resizable,
        "draggable": original.config.draggable,
        "height": original.size.height,
        "width": original.size.width,
      ...
      //$(this.element[0]).removeData("dialog-extend-overlay");
      return this._trigger("_restore_minimized");
    },
    ...
    _minimize_removeOverlay: function() {
      ...
      $(this).dialog("widget").appendTo($('#workspace'));
