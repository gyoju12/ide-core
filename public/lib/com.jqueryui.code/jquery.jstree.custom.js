	...
	$.jstree.core.prototype = {
		...
		get_node: function(obj, as_dom) {
			...
			try {
				...
				if (as_dom) {
					// Goorm change start
					obj = obj.id === '#' ? this.element : $('[id="' + obj.id.replace(/([\"\[\]\!\@\#\$\%\^\&\*\(\)])/g, '\\$1') + '"]', this.element);
		...
		redraw_node: function(node, deep, is_callback) {
			...
			var _id = ("0123456789".indexOf(obj.id[0]) !== -1 ? '\\3' + obj.id[0] + ' ' + obj.id.substr(1).replace($.jstree.idregex, '\\$&') : obj.id.replace($.jstree.idregex, '\\$&'));
			node = !document.querySelector ? document.getElementById(obj.id) : this.element[0].querySelector('[id="' + _id.replace(/([\"\[\]\!\@\#\$\%\^\&\*\(\)])/g, '\\$1') + '"]'); //, this.element);
			// Goorm change end
			if (!node) {
				deep = true;
		...
		_open_to: function(obj) {
			...
			return $('[id="' + obj.id.replace(/([\"\[\]\!\@\#\$\%\^\&\*\(\)])/g, '\\$1') + '"]', this.element);
			// Goorm change end
		},
		/**
		...
		get_state: function() {
			...
			for (i in this._model.data) {
				if (this._model.data.hasOwnProperty(i)) {
					if (i !== '#') {
						if (this._model.data[i].state) { // jeongmin: because of 'children_d of undefined' error, state may be undefined
