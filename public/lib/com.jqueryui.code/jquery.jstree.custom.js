...
(function(factory) {
	...
	$.jstree.core.prototype = {
		...
		init: function(el, options) {
			...
			this._element = this.element; // jeongmin: prepare for the null
		...
		trigger: function(ev, data) {
			...
			data.instance = this;
			if (!this.element) this.element = this._element; // jeongmin: if this.element is null, substitute as emergency element
		...
		_load_node: function(obj, callback) {
			...
			if ($.isFunction(s)) {
				return s.call(this, obj, $.proxy(function(d, cb) { // jeongmin: cb == code after callback.call in core.data in utility.treeview.js
					if (d === false) {
						callback.call(this, false);
					}
					this[typeof d === 'string' ? '_append_html_data' : '_append_json_data'](obj, typeof d === 'string' ? $(d) : d, function(status) {
						callback.call(this, status);
						cb(this._model.data); // jeongmin: Callback is called after tree data is loaded(worker is terminated). this._model.data is need for obj update
					});
		...
		_append_html_data: function(dom, data, cb) {
			...
			if (!dom) return; // jeongmin: getting node fails
		...
		_append_json_data: function(dom, data, cb, force_processing) {
			...
			var w = null,
				...
				rslt = function(rslt, worker) {
					...
					if (cb) { // jeongmin: some _append_json_data doesn't have callback
						cb.call(this, true);
					}
				};
		...
		_redraw: function() {
			...
			if (this._model.force_full_redraw) {
				if (this.get_container_ul()[0]) { // jeongmin: this can be undefined
		...
		this.bind = function() {
			this.element
				.on('model.jstree', $.proxy(function(e, data) {
					...
					for (i = 0, j = dpc.length; i < j; i++) {
						c = 'default';
						if (m[dpc[i]]) { // jeongmin: check m[dpc[i]] also
		...
		this.bind = function() {
			...
			this.element
				.on('ready.jstree set_state.jstree', $.proxy(function() {
					if (!this.settings.core.themes.dots) { // jeongmin: we want to show dots in wholerow
						this.hide_dots();
					}
				}, this))
