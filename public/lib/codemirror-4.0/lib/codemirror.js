...
(function(mod) {
  ...
  function updateLineGutter(cm, lineView, lineN, dims) {
    ...
    if (cm.options.lineNumbers || markers) {
      ...
      if (markers) {
        ...
        cm.options.gutters.map(function (o){
          var o_split = o.split(" ");
          __length += o_split.length;
          __ids = __ids.concat(o_split);
        });

        for (var k = 0; k < __length; ++k) {
          var id = __ids[k], found = markers.hasOwnProperty(id) && markers[id];
          if (found)
  ...
  function endOperation(cm) {
    ...
    if (cm.be_changed !== false) cm.be_changed = true;
    ...
    if (op.changeObjs && cm.be_changed) {
  ...
  function filterChange(doc, change, update) {
    ...
    return {from: obj.from, to: obj.to, text: obj.text, origin: obj.origin, be_changed: change.be_changed};
  }
  ...
  function makeChange(doc, change, ignoreReadOnly) {
    if (doc.cm) {
      doc.cm.be_changed = change.be_changed;
    ...
    if (split) {
      for (var i = split.length - 1; i >= 0; --i)
        makeChangeInner(doc, {from: split[i].from, to: split[i].to, text: i ? [""] : change.text, be_changed: change.be_changed});
    } else {
  ...
  function replaceRange(doc, code, from, to, origin, be_changed) {
    if (!to) to = from;
    if (cmp(to, from) < 0) { var tmp = to; to = from; from = tmp; }
    if (typeof code == "string") code = splitLines(code);
    if (be_changed !== false) be_changed = true;
    makeChange(doc, {from: from, to: to, text: code, origin: origin, be_changed: be_changed});
  }
  ...
  Doc.prototype = createObj(BranchChunk.prototype, {
    ...
    replaceRange: function(code, from, to, origin, be_changed) {
      from = clipPos(this, from);
      to = to ? clipPos(this, to) : from;
      replaceRange(this, code, from, to, origin, be_changed);
    },
