	...
	open: function(filepath, filename, filetype, editor, __options, callback) {
		...
		function do_open() {
			...
			if (i > -1) {
				...
				$(core).one(filepath + '/' + filename + options.index + '.window_loaded', function(e, __window) {
					if (callback && typeof(callback) === 'function') {
						callback(__window);
					}
				});
				...
				//$.get("project/get_list", postdata, function (project_data) {
					...
					synch_with_fs: function(condition) {
						...
						if (condition) {
							...
							if (condition.change == 'dialog_mv') {
								for (i = window_list.length - 1; i >= 0; i--) {
									if ((window_list[i].title).indexOf(condition.ori_path + '/' + condition.ori_file) === 0) { // jeongmin: there should be / between path and file
										//close all files children of not existed folder
										window_list[i].is_saved = true;
										window_list[i].tab.is_saved = true;
										// window_list[i].close();

										// this.close_by_index(i, i);
										window_list[i].tab.close();
										window_list[i].close(); // jeongmin: this code should be after above code! Because tab is in window_list[i], so don't close window before closing tab.
									}
					...
					close_by_index: function(window_target_idx, tab_target_idx) {
						...
						if (window_list[window_target_idx].is_saved) {
							...
							if (window_target_idx != -1) {
								//window_list[window_target_idx].is_saved=true;
								window_list[window_target_idx].close();
							}
						} else {
							confirmation_save.init({
								message: "\"" + window_list[window_target_idx].filename + "\" " + core.module.localization.msg.confirmation_save_message,
								yes_text: core.module.localization.msg.confirmation_yes,
								cancel_text: core.module.localization.msg.confirmation_cancel,
								no_text: core.module.localization.msg.confirmation_no,
								title: "Close...",
								...
								no: function() {
									window_list[window_target_idx].is_saved = true;
									tab_list[tab_target_idx].is_saved = true;
									self.close_by_title(window_list[window_target_idx].title);
								}
							});
