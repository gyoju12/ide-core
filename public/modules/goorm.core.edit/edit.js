...
goorm.core.edit.prototype = {
	...
	get_editor_idx: function(total_path) {
		...
		for (var i = 0; i < window_manager.window.length; i++) {
			if (window_manager.window[i].filepath + window_manager.window[i].filename == total_path) {
				return i;
			}
		}
		return -1;
	...
	codemirror_events: function() {
		...
		cm_editor.on('change', function(i, e, a) { // i = CodeMirror object, e = change informations;
			...
			if (self.init_change && ~my_idx) {
				...
				var target_tab = core.module.layout.workspace.window_manager.tab[my_idx];
		...
		__target.on('keydown', function(e) {
			...
			if (e.keyCode == 8 && only) {
				...
				if (only) {
					self.str_selection = self.char_right;
				}
				if (e.keyCode == 219) {
					self.pressed_key = '{';
				}
		...
		cm_editor.on('dragover', function(cm, e) {
			...
			var top = e.pageY - parseInt(code_mirror.offset().top, 10) - 3;
			var left = e.pageX - parseInt(code_mirror.offset().left, 10) - 3;
	...
	get_editor_preference: function(pref_name) {
		...
		if (this.preference[preference_type] == 'true') {
			this.preference[preference_type] = true;
		} else if (this.preference[preference_type] == 'false') {
			this.preference[preference_type] = false;
		}
	...
	set_bookmark: function(bookmark_list, line) { // line: that we want to set bookmark
		...
		if (!this.editor) {
			return; //if there is no active editor, do nothing
		}
	...
	restore_unsaved_file_from_local: function() {
		...
		if (!localStorage.unsaved_data) {
			return false;
		}
		var unsaved_data = JSON.parse(localStorage.unsaved_data);
		if (!unsaved_data || unsaved_data.length === 0) {
			return false;
		}
