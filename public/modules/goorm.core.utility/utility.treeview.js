...
goorm.core.utility.treeview.prototype = {
	...
	bind: function() {
		...
		this.tree.on('loaded.jstree', function(event, data) {
			...
			if (_this.opened_node[data.node.id]) { // we have opened node list
				var state = _this.tree.jstree('get_state');

				state.core.open = state.core.open.concat(_this.opened_node[data.node.id]); // show opened children of current opening node

				_this.tree.jstree('set_state', state);
			...
			var open = _this.tree.jstree('get_state').core.open;

			for (var i = 0; i < open.length; i++) {
				if (!~open[i].indexOf(data.node.id)) { // not children
					open.splice(i--, 1); // array is spliced, so remove index too
				}
			}

			_this.opened_node[data.node.id] = open; // save current closing node's opened children
	...
	get_children: function(node) {
		...
		if ($.isArray(node.children)) {
			...
			return this.get_node(node.children);
		}
	},
